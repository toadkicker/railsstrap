shared_examples_for 'the panel helper' do
  ...
  all_tests_pass_with 'the :variant panel option'
...
shared_examples_for 'the :body panel option' do
  ...
  specify 'given neither content nor a block, it is displayed as the content' do
    expect(railsstrap.panel(body: 'Body')).to match html
  end
end
...
shared_examples_for 'the :variant panel option' do
  Railsstrap::Panel.variants.each do |variant, variant_class|
    specify %Q{set to :#{variant}, adds the class "#{variant_class}"} do
      html = %r{^<div class="panel #{variant_class}">.*</div>$}m
      expect(panel: {variant: variant}).to generate html
    end
  end
end
...
shared_examples_for 'the panel wrapped in panel_row' do
  specify 'wraps the panel <div> in a grid <div> with the :column_class class of the panel row' do
    ...
    railsstrap.panel_row(column_class: 'col-sm-4') { expect(:panel).to generate html }
  end
end