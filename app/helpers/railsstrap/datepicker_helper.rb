...
module Railsstrap
  module DatePickerHelper
    ...
    def date_picker(options = {})
      ...
      input = tag opts[:input_tag], type: opts[:input_type], id: opts[:id], class: opts[:input_class]
      icon_wrapper = content_tag opts[:icon_wrapper_tag], class: opts[:icon_wrapper_class] do
        icon(opts[:icon])
      end
      content_tag(opts[:wrapper_tag], class: opts[:wrapper_class], data: opts[:data]) {
    ...
    def date_picker_javascript(options = {})
      ...
      javascript_tag "jQuery(function () { jQuery(#{opts[:selector]}).datetimepicker(#{opts.to_json});});"
    end
  end
end